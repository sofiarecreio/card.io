      ...
      <div className="grid gap-5 lg:grid-cols-12">
        ...
        <BiCard
          ...
          action={
            <select
              ...
              className="h-8 rounded-full border border-border bg-secondary/60 px-3 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-ring"
            >
              {patients.map((p) => (
        ...
        >
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {forms.map((f) => {
              const alerts = f.sections.flatMap((s) => s.fields).filter((x) => x.flag === "alert").length;
              const warns  = f.sections.flatMap((s) => s.fields).filter((x) => x.flag === "warn").length;
              const StatusIcon = f.status === "completo" ? CheckCircle2 : f.status === "parcial" ? Clock : CircleDashed;
              const statusCls = f.status === "completo" ? "text-success" : f.status === "parcial" ? "text-warning" : "text-muted-foreground";
              return (
                <button
                  key={f.key}
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="mt-0.5 text-[11px] text-muted-foreground">{f.short}</div>
                  </div>
                  <div className="mt-auto flex items-center justify-between pt-2 text-[10px]">
                    <span className="text-muted-foreground">Atualizado {f.updated}</span>
              ...
              <div className="mt-2 space-y-5">
                {activeForm.sections.map((sec) => (
                  <section key={sec.title}>
                    <h4 className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{sec.title}</h4>
                    <dl className="divide-y divide-border overflow-hidden rounded-xl border border-border">
                      {sec.fields.map((f, i) => {
                        const flagCls =
                          f.flag === "alert" ? "bg-danger/10 border-l-2 border-danger"
                          : f.flag === "warn" ? "bg-warning/10 border-l-2 border-warning"
                          : "";
                        const valCls =
                          f.flag === "alert" ? "text-danger font-semibold"
                          : f.flag === "warn" ? "text-warning font-semibold"
                          : "text-foreground";
                        return (
                          <div key={i} className={`flex items-start justify-between gap-3 px-3 py-2 text-sm ${flagCls}`}>
                            <dt className="text-muted-foreground">{f.label}</dt>
                            <dd className={`text-right ${valCls}`}>{f.value}</dd>
                          </div>
                        );
                      })}
                    </dl>
