...
export const Route = createFileRoute("/equipe")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "KPIs e monitoramento da População de estudo com insuficiência cardíaca." },
    ],
  }),
...
function TeamDashboard() {
  ...
  const indicatorMatch = indicatorSearch.trim() ? findBestPatient(indicatorSearch, searchPool) : selectedPatient;
  ...
  const avgAdherence = Math.round(filteredPatients.reduce((sum, p) => sum + p.adherence, 0) / totalFiltered) || 0;
  const avgSelfCare = Math.round(filteredPatients.reduce((sum, p) => sum + p.selfCare, 0) / totalFiltered) || 0;
  ...
  return (
    <AppShell profile="team">
      <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
            Visão da População de estudo com foco em prioridade clínica, evolução longitudinal e alertas acionáveis.
          </p>
        </div>
        <FilterBar
          filters={filters}
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Kpi label="Pacientes filtrados" value={filteredPatients.length} delta={`${patients.length} na base`} trend="flat" icon={<Users className="h-4 w-4" />} />
        <Kpi label="Alertas críticos" value={criticalAlerts.length} delta="prioridade de hoje" trend="down" tone="danger" icon={<AlertTriangle className="h-4 w-4" />} />
        <Kpi label="Sem atualização" value={stalePatients.length} delta="resposta atrasada" trend="down" tone="warning" icon={<ClipboardList className="h-4 w-4" />} />
        <Kpi label="Adesão média" value={`${avgAdherence}%`} delta={`autocuidado ${avgSelfCare}%`} trend="up" tone="success" icon={<HeartPulse className="h-4 w-4" />} />
      </div>

      <div className="grid gap-5 lg:grid-cols-12">
        <BiCard className="lg:col-span-5" title="Alertas críticos" subtitle="Pacientes com maior risco de piora nas últimas 24h" accent="danger">
          <div className="space-y-3">
            {criticalAlerts.length === 0 && <EmptyState text="Nenhum alerta crítico para os filtros atuais." />}
            {criticalAlerts.map((p) => (
              <div key={p.id} className="rounded-2xl border border-danger/25 bg-danger/5 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-sm font-semibold">{p.name}</div>
                    <div className="text-[11px] text-muted-foreground">{p.id} · {p.lastResponse} · {getProfessional(p.id)}</div>
                  </div>
        ...
        <BiCard className="lg:col-span-7" title="Pacientes que precisam de atenção" subtitle="Ordenado por risco, tendência de piora e atraso de resposta">
          <div className="space-y-2">
            {attentionPatients.length === 0 && <EmptyState text="Nenhum paciente prioritário nos filtros atuais." />}
        ...
        >
          <div className="-mx-2 overflow-x-auto">
              ...
              <tbody>
                {filteredPatients.map((p) => (
                  <tr key={p.id} className="border-t border-border transition hover:bg-secondary/40">
                    ...
                    <td className="px-2 py-2.5"><ProgressCell value={p.selfCare} /></td>
                    <td className="px-2 py-2.5"><ProgressCell value={p.adherence} /></td>
                    <td className="px-2 py-2.5 text-xs">{p.hr} bpm</td>
                    <td className="px-2 py-2.5 text-xs">
                      <span className={p.spo2 < 93 ? "font-semibold text-danger" : ""}>{p.spo2}%</span>
                    </td>
                    <td className="px-2 py-2.5 text-[11px] text-muted-foreground">{p.lastResponse}</td>
                    <td className="px-2 py-2.5 text-[11px] text-muted-foreground">{getProfessional(p.id)}</td>
                    <td className="px-2 py-2.5">
            ...
            {filteredPatients.length === 0 && <EmptyState text="Nenhum paciente encontrado com os filtros atuais." />}
          </div>
        </BiCard>

        <BiCard className="lg:col-span-4" title="Distribuição por risco clínico" subtitle="Classificação da População de estudo filtrada">
          <div className="space-y-4">
            {riskCounts.map((item) => {
              const pct = Math.round((item.value / totalFiltered) * 100);
              const barColor = item.risk === "high" ? "bg-danger" : item.risk === "medium" ? "bg-warning" : "bg-success";
              return (
                <div key={item.risk}>
                  <div className="mb-1 flex items-center justify-between text-xs">
                    <span className="font-semibold">{item.label}</span>
                    <span className="text-muted-foreground">{item.value} pacientes · {pct}%</span>
                  </div>
                  <div className="h-2 overflow-hidden rounded-full bg-muted">
                    <div className={`h-full rounded-full ${barColor}`} style={{ width: `${pct}%` }} />
                  </div>
                  <p className="mt-1 text-[11px] leading-snug text-muted-foreground">{riskDescriptions[item.risk]}</p>
                </div>
              );
            })}
          </div>
        ...
        <BiCard className="lg:col-span-12" title="Pacientes sem resposta ou sem atualização recente" subtitle="Acompanhamento ativo recomendado antes da análise dos gráficos">
            ...
            {stalePatients.map((p) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-sm font-semibold">{p.name}</div>
                    <div className="mt-0.5 text-[11px] text-muted-foreground">{p.id} · {p.lastResponse}</div>
                  </div>
                  <Clock className="h-4 w-4 text-warning" />
                </div>
                <div className="mt-3 text-xs text-muted-foreground">Abrir prontuário e revisar a última evolução.</div>
              </a>
            ))}
            {stalePatients.length === 0 && <EmptyState text="Todos os pacientes filtrados atualizaram recentemente." />}
          </div>
        </BiCard>
        ...
        >
          <p className="mb-3 text-xs text-muted-foreground">
            Busca tolerante a pequenos erros de grafia. Mostra tendência de FC e peso para detectar ganho ponderal e taquicardia persistente.
          </p>
        ...
        <BiCard className="lg:col-span-5" title="Indicadores de piora recente" subtitle="Quantidade de pacientes por sinal clínico prioritário">
          <p className="mb-3 text-xs text-muted-foreground">
            Resume quais alterações estão mais presentes na população filtrada e orienta o tipo de intervenção.
          </p>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={worseningData} layout="vertical" margin={{ top: 10, right: 10, left: 32, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" horizontal={false} />
              <XAxis type="number" tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} axisLine={false} tickLine={false} />
              <YAxis type="category" dataKey="indicator" tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} axisLine={false} tickLine={false} width={88} />
              <Tooltip contentStyle={tooltipStyle} />
        ...
        <BiCard className="lg:col-span-6" title="Fila de ação clínica" subtitle="Prioridade prática para contato e revisão">
          <p className="mb-3 text-xs text-muted-foreground">
            Organiza a carga assistencial em grupos acionáveis, evitando mistura de métricas clínicas e operacionais.
          </p>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={actionQueueData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
              <XAxis dataKey="queue" tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={tooltipStyle} />
              <Bar dataKey="patients" name="Pacientes" fill="var(--primary)" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </BiCard>

        <BiCard className="lg:col-span-6" title="Atualização remota por risco" subtitle="Quem respondeu recentemente e quem exige busca ativa">
          <p className="mb-3 text-xs text-muted-foreground">
            Mostra a cobertura de atualização por nível de risco clínico para priorizar pacientes silenciosos.
          </p>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={updateCoverageData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
              <XAxis dataKey="risk" tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 11, fill: "var(--muted-foreground)" }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={tooltipStyle} />
              <Legend wrapperStyle={{ fontSize: 11 }} />
              <Bar dataKey="recentes" name="Atualização recente" fill="var(--success)" radius={[6, 6, 0, 0]} />
              <Bar dataKey="atrasados" name="Sem atualização" fill="var(--warning)" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </BiCard>
      </div>
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-3 shadow-[var(--shadow-card)]">
      ...
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
        <select value={filters.period} onChange={(event) => onChange("period", event.target.value as PeriodFilter)} className="h-9 rounded-xl border border-border bg-background px-3 text-xs focus:outline-none focus:ring-2 focus:ring-ring">
          <option value="7">Últimos 7 dias</option>
          <option value="30">Últimos 30 dias</option>
          <option value="90">Últimos 90 dias</option>
        </select>
        <select value={filters.risk} onChange={(event) => onChange("risk", event.target.value as RiskFilter)} className="h-9 rounded-xl border border-border bg-background px-3 text-xs focus:outline-none focus:ring-2 focus:ring-ring">
        ...
        <select value={filters.response} onChange={(event) => onChange("response", event.target.value as ResponseFilter)} className="h-9 rounded-xl border border-border bg-background px-3 text-xs focus:outline-none focus:ring-2 focus:ring-ring">
        ...
        <select value={filters.professional} onChange={(event) => onChange("professional", event.target.value)} className="h-9 rounded-xl border border-border bg-background px-3 text-xs focus:outline-none focus:ring-2 focus:ring-ring">
          <option value="all">Profissional: todos</option>
          {professionalFollowUp.map((item) => (
            <option key={item.professional} value={item.professional}>{item.professional}</option>
          ))}
        </select>
        <button onClick={onClear} className="flex h-9 items-center justify-center gap-2 rounded-xl border border-border bg-secondary/60 px-3 text-xs font-semibold text-muted-foreground transition hover:text-foreground">
    ...
    <div className="flex flex-col gap-3 rounded-2xl border border-border bg-secondary/25 p-3 md:flex-row md:items-center md:justify-between">
      <div className="min-w-0">
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {patient.id} · {patient.lastResponse} · autocuidado {patient.selfCare}% · adesão {patient.adherence}%
        </div>
      </div>
      <a href={`/prontuario/${patient.id}`} className="rounded-xl border border-border bg-card px-3 py-2 text-center text-xs font-semibold transition hover:bg-accent">
        Abrir prontuário
      </a>
    </div>
  );
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="h-1.5 w-20 overflow-hidden rounded-full bg-muted">
        <div className={`h-full rounded-full ${value < 60 ? "bg-danger" : value < 75 ? "bg-warning" : "bg-success"}`} style={{ width: `${value}%` }} />
      </div>
...
function priorityScore(patient: Patient) {
  return (patient.risk === "high" ? 40 : patient.risk === "medium" ? 20 : 0)
...
function buildActionQueueData(list: Patient[]) {
  return [
    { queue: "Contato hoje", patients: list.filter((p) => p.risk === "high" && (p.trend === "down" || isStale(p))).length },
    { queue: "Revisar 24h", patients: list.filter((p) => p.risk === "high" && p.trend !== "down" && !isStale(p)).length },
    { queue: "Acompanhar 48h", patients: list.filter((p) => p.risk === "medium" || p.selfCare < 70).length },
...
function patientMatches(patient: Patient, normalizedQuery: string) {
  ...
  return normalizedName
    .split(/\s+/)
    .some((token) => levenshtein(token, normalizedQuery) <= Math.max(1, Math.floor(normalizedQuery.length / 3)));
}
